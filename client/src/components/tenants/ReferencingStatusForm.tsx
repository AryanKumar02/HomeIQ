...
const ReferencingStatusForm: React.FC<ReferencingStatusFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.reference && formData.reference !== tenant.referencing?.reference) updateData.reference = formData.reference
      if (formData.outcome !== tenant.referencing?.outcome) updateData.outcome = formData.outcome
      if (formData.conditions !== tenant.referencing?.conditions) updateData.conditions = formData.conditions
      if (formData.notes !== tenant.referencing?.notes) updateData.notes = formData.notes
      if (formData.completedDate !== tenant.referencing?.completedDate) updateData.completedDate = formData.completedDate
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to update referencing status'
  ...
  const getCurrentOutcome = () => {
    return REFERENCING_OUTCOMES.find(outcome => outcome.value === formData.outcome) || REFERENCING_OUTCOMES[0]
  }

  const getCurrentStatus = () => {
    return REFERENCING_STATUSES.find(status => status.value === formData.status) || REFERENCING_STATUSES[0]
  }
        ...
        <Typography variant="h6" gutterBottom sx={{ fontWeight: 600, mb: 3 }}>
          ...
          <Box sx={{ mt: 3, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
            <Typography variant="body2" color="text.secondary" gutterBottom>
              <strong>Current Status:</strong> {getCurrentStatus().label} • <strong>Outcome:</strong> {getCurrentOutcome().label}
            </Typography>
