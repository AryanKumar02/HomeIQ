...
export async function login(email: string, password: string, rememberMe: boolean = false): Promise<AuthResponse> {
  try {
    ...
    const res = await axios.post<AuthResponse>('/api/v1/auth/login', { email, password, rememberMe }, {
...
export async function logout(): Promise<AuthResponse> {
  try {
    ...
    const res = await axios.post<AuthResponse>('/api/v1/auth/logout', {}, {
      withCredentials: true,
    });
...
export async function signup(firstName: string, secondName: string, email: string, password: string): Promise<AuthResponse> {
  try {
    console.log('SIGNUP API CALL:', '/api/v1/auth/register', { firstName, secondName, email });
    const res = await axios.post<AuthResponse>('/api/v1/auth/register', { firstName, secondName, email, password }, {
      withCredentials: true,
    });
