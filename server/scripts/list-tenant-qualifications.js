...
const listTenantQualifications = async () => {
  try {
    ...
    const needsReview = tenants.filter(
      t => t.getQualificationStatus().status === 'needs-review',
    ).length;
    const notQualified = tenants.filter(
      t => t.getQualificationStatus().status === 'not-qualified',
    ).length;
